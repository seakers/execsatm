...
class ObservationOpportunity:
    ...
    def __init__(self,
        ...
        **Arguments:**
        ...
        assert isinstance(tasks, Iterable) and all(isinstance(task, GenericObservationTask) for task in tasks), \
            "tasks must be a non-empty set of `GenericObservationTask`."
        ...
        # assert all(accessibility.overlaps(t_acc) for t_acc in task_accessibility.values()), \
        #     "The observation opportunity's accessibility interval must be a subset of each task's accessibility interval."
    ...
    def is_mutually_exclusive(self, other : 'ObservationOpportunity') -> bool:
        ...
        # Rule 0: if they are the same observation opportunity or their accessibility intervals do not overlap, 
        # they are not mutually exclusive (i.e. they can be scheduled at different times)
        if other == self or not self.accessibility.overlaps(other.accessibility):
            return False

        # Rule 1: same parent task(s) with overlapping accessibility; 
        # cannot schedule multiple observations of the same task at the same pass
        if len(self.tasks.intersection(other.tasks)) > 0:
            return True

        # Rule 2: same event, same instrument, same pass — one observation per event per instrument per pass.
        # Different-instrument obs opps for the same event are NOT exclusive (e.g. TIR + VNIR imaging simultaneously).
        if self.instrument_name == other.instrument_name:
            my_event_ids = {task.event.id for task in self.tasks
                            if hasattr(task, 'event') and task.event is not None}
            other_event_ids = {task.event.id for task in other.tasks
                               if hasattr(task, 'event') and task.event is not None}
            if my_event_ids and other_event_ids and my_event_ids & other_event_ids:
                return True
    ...
    def merge(self, 
        ...
        **Arguments:**
        ...
        assert min_duration_req <= merged_accessibility.span(), \
            "minimum duration requirements exceed accessibility span."
...
class DummyObservationOpportunity(ObservationOpportunity):
    def __init__(self, 
                 instrument_name : str,
                 accessibility : Interval,
                 slew_angles : Interval,
        ...
        """
        ## Dummy observation opportunity
        Represents a dummy observation opportunity that can be used for initialization or as a placeholder. 
        This is a special case of `ObservationOpportunity` where there are no parent tasks.
        ...
        **Arguments:**
        ...
        # initialize the observation opportunity with no parent tasks and specified parameters
        super().__init__(
            tasks = set(),
            instrument_name = instrument_name,
            task_accessibility = dict(),
            task_slew_angles = dict(),
            task_min_duration = dict(),
            accessibility = accessibility,
            slew_angles = slew_angles,
            min_duration = min_duration,
            max_duration = max_duration,
